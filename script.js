...
const elements = {
    ...
    copyPeerId: document.getElementById('copyPeerId'),
...
const ui = {
    ...
    updateConnectionStatus: (status) => {
        ...
        elements.statusText.innerHTML = statusIndicator;
        elements.peerIdText.textContent = state.peer?.id || 'Not set';
    },
    ...
    copyPeerId: () => {
        ...
        if (peerId) {
            navigator.clipboard.writeText(peerId).then(() => {
                ui.showAlert('Peer ID copied to clipboard!', 'success');
            }).catch(() => {
                ui.showAlert('Failed to copy Peer ID', 'danger');
            });
        } else {
            ui.showAlert('No Peer ID available to copy', 'danger');
        }
    }
};
...
const peerFunctions = {
    initialize: (customId) => {
        ...
        state.peer.on('open', (id) => {
            ...
            elements.peerIdText.textContent = id;
...
const initApp = () => {
    ...
    // Copy Peer ID event
    elements.copyPeerId.addEventListener('click', ui.copyPeerId);
};
